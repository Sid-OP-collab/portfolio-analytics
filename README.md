...
!\[Portfolio value over time](reports/portfolio\_value\_sample.png)
...
python src/valuation.py data/sample\_transactions.csv   # portfolio value over time
python src/returns.py data/sample\_transactions.csv      # TWR and XIRR
python src/risk.py data/sample\_transactions.csv         # volatility, Sharpe, drawdown, attribution
python src/plot\_portfolio.py data/sample\_transactions.csv  # saves a chart to reports/
```
...
and point any script at it instead of `data/sample\_transactions.csv`.
...
A cleaning script (`src/clean\_transactions.py`) converts a raw broker
...
loader.py       Reads and validates a transaction CSV
    -> ledger.py    FIFO lot matching; daily holdings + realised P\&L
        -> valuation.py   Joins holdings to prices -> daily portfolio value
            ...
            -> plot\_portfolio.py   Chart generation
...
`total\_value`, which jumps every time money is deposited. The code read
...
instead. `tests/test\_risk.py` has explicit regression tests pinning this
...
This repo ships with `data/sample\_transactions.csv`, a synthetic file
